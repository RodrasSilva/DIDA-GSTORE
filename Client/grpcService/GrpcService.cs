...
namespace DIDA_GSTORE.grpcService {
    public class GrpcService {
        ...
        private void RemoveClientUrl(string url) {
            foreach (var parId in _clientLogic.ServerList.Keys) _clientLogic.ServerList[parId].Remove(url);
        }
        ...
        private string ReadAdvanced(string partitionId, string objectId, string serverId) {
            ...
            try {
                ...
                if (serverId.Equals("-1")) return _previousStorage.GetAndUpdate(partitionId, objectId);
        ...
        public Dictionary<string, List<ListServerResult>> ListGlobal() {
            var result = new Dictionary<string, List<ListServerResult>>();
    ...
    public class PreviousStorage {
        ...
        private Dictionary<PartitionObject, ReadAdvancedResponse> _previousValues =
            new Dictionary<PartitionObject, ReadAdvancedResponse>();
        ...
        public void WriteToCache(string partitionId, string objectId, string objectValue, int timestamp) {
            ...
            var r = new ReadAdvancedResponse {ObjectValue = objectValue, Timestamp = timestamp};
