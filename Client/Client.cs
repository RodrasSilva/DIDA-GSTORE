...
namespace Client {
    public static class Client {
        ...
        public static void Main(string[] args) {
            ...
            var partitions = new string[args.Length - 4];
            for (var i = 4; i < args.Length; i++) partitions[i - 4] = args[i];

            foreach (var item in args) Console.WriteLine(item.ToString());
            foreach (var item in partitions) Console.WriteLine(item.ToString());
