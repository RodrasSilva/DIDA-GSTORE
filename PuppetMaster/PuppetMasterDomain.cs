...
namespace PuppetMasterMain {
    ...
    public class PuppetMasterDomain {
        ...
        public void RemoveServer(string serverId) {
            ...
            foreach (var partitionInfo in Partitions) partitionInfo.serverIds.Remove(serverId);
        }
        ...
        public List<ServerInfoMessage> GetServerInfoMessages() {
            var serverInfosDtos = new List<ServerInfoMessage>();
        ...
        public List<PartitionInfoMessage> GetPartitionInfo(string serverId) {
            var partitionInfoDTOs = new List<PartitionInfoMessage>();
        ...
        public List<PartitionClientMessage> GetPartitionClientInfo() {
            var partitionInfoDTOs = new List<PartitionClientMessage>();
            foreach (var partition in Partitions) {
                var serverUrls = new List<string>();
                foreach (var serverId in partition.serverIds) serverUrls.Add(ServerServices[serverId].Url);
        ...
        private void ExecuteSetup(List<ICommand> commands) {
            try {
                ...
                foreach (var command in commands) command.Execute(this);

                var serverInfosDtos = GetServerInfoMessages();

                foreach (var serverId in ServerServices.Keys) {
                    var partitionInfoDTOs = GetPartitionInfo(serverId);
